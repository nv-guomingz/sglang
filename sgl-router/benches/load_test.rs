//! Standalone load testing tool for SGLang router
//!
//! Usage:
//!   cargo run --release --bin load_test -- [requests] [workers] [batch_size]
//!
//! Examples:
//!   cargo run --release --bin load_test -- 1000 5 100
//!   cargo run --release --bin load_test -- 10000 10 200
...
// Import test utilities for mock workers
...
async fn run_load_test(config: LoadTestConfig) {
    ...
    // For PD mode, we need to include worker URLs at creation time
    ...
    while processed < config.requests {
        ...
        for i in 0..batch_size {
            ...
            let future = async move {
                ...
                // Create payload using common utility
                ...
                if resp.status().is_success() {
                    ...
                    // Try to track which worker handled this request (simplified)
                    let worker_idx = req_num % worker_reqs_ref.len();
    ...
    let total_completed = total_successful + total_failed;
    let total_latency = total_latency_ns.load(Ordering::Relaxed);

    // Clear the progress bar line and move to results
    println!();
    println!("\n{}", "=".repeat(80));
    ...
    println!("\n📊 Summary:");
    println!("  Total requests:       {}", config.requests);
    println!(
        "  Completed:           {} ({:.1}%)",
        total_completed,
        (total_completed as f64 / config.requests as f64) * 100.0
    );
    println!(
        "  Successful:          {} ({:.1}%)",
        total_successful,
    ...
    println!(
        ...
        total_failed,
        if total_completed > 0 {
            (total_failed as f64 / total_completed as f64) * 100.0
        } else {
            0.0
        }
    );
    ...
    if total_successful > 0 {
        let avg_latency_ns = total_latency as f64 / total_successful as f64;
        let avg_latency_ms = avg_latency_ns / 1_000_000.0;
        println!("  Average latency:     {:.2}ms", avg_latency_ms);

        // Estimate percentiles (simplified - in production use proper histogram)
        let estimated_p50 = avg_latency_ms * 0.8;
        let estimated_p95 = avg_latency_ms * 1.5;
        let estimated_p99 = avg_latency_ms * 2.0;
        println!("  Estimated P50:       {:.2}ms", estimated_p50);
        println!("  Estimated P95:       {:.2}ms", estimated_p95);
        println!("  Estimated P99:       {:.2}ms", estimated_p99);
    }

    println!("\n🔄 Worker Distribution:");
    let mut _total_worker_requests = 0;
    for (i, count) in worker_requests.iter().enumerate() {
        let requests = count.load(Ordering::Relaxed);
        _total_worker_requests += requests;
        let percentage = if total_successful > 0 {
            (requests as f64 / total_successful as f64) * 100.0
        } else {
            0.0
        };

        let worker_type_str = if config.routing_mode == "pd" {
            if i < config.prefill_workers {
                " (Prefill)"
            } else {
                " (Decode)"
    ...
    println!("  Batch size:          {}", config.batch_size);
    println!("  Worker delay:        {}ms", config.worker_delay_ms);
    println!("  Router port:         {}", config.router_port);
    println!("  Routing mode:        {}", config.routing_mode);
    println!("  Policy:              {}", config.policy);
